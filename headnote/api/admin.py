...
@router.post("/cron/send-daily-causelist",
             summary="Send each lawyer their evening cause-list link (WhatsApp + email)")
def cron_send_daily_causelist(
    ...
    """Wire to ONE evening cron with the ADMIN_TOKEN — e.g. 18:30 IST (13:00 UTC).
    One link per lawyer settles today (upload the marked sheet) AND prints tomorrow's
    list. Only lawyers with cases today or tomorrow are messaged; dry_run previews.
      curl -X POST 'https://headnote.in/admin/cron/send-daily-causelist' \\
           -H "Authorization: Bearer $ADMIN_TOKEN"
    """
    _require_admin(authorization)
