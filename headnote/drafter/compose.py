...
def _generate_document(template: dict, collected: dict, lang: str) -> str:
    ...
    sys = (
        f"You are a senior Indian advocate drafting a "
        ...
        "as given.\n\n"
        "TOP-SECTION FOR A COURT FILING — this OVERRIDES any header / cause-title in "
        "the FORMAT SPEC below, but ONLY for a document actually filed before a court.\n"
        "SKIP THIS BLOCK ENTIRELY, and follow the FORMAT SPEC's own opening, whenever "
        "the document is NOT a court filing — i.e. it is addressed to an office rather "
        "than a judge (a police station, a Collector/SDM/Tahsildar/RTO, a Registrar, a "
        "bank or an employer), or it is sent to the other side (a विधिक नोटिस / legal "
        "notice), or it is executed between parties (an agreement or deed), or it "
        "stands alone (a वकालतनामा, a शपथ पत्र / affidavit, a mention memo). Such a "
        "document has NO court name, NO case number, NO विरुद्ध and NO opposing-party "
        "block; forcing a cause-title onto it produces a document the advocate cannot "
        "send. For a COURT FILING, open with exactly these lines, in order:\n"
        "  1. Side-line (centered, small): who it is on behalf of — e.g. Hindi "
        "'बंदी की ओर से' / 'आवेदक की ओर से'; English 'On behalf of the Applicant'. "
        "Render 'बंदी' as 'Applicant' in English, NEVER 'Detenue' (habeas corpus excepted).\n"
        "  2. COURT NAME — the single most prominent line, centered, spanning the full "
        "width — taken from the court field for the matter's OWN State (e.g. "
        "'माननीय उच्च न्यायालय <राज्य> खण्डपीठ <स्थान>'); if the State is unknown leave it blank (____), never default to any State.\n"
        "  3. Case-number line (centered): case-type + number + year; the type matches the "
        "forum — एम.सी.आर.सी. at High Court, प्रकरण क्रमांक at Sessions/Magistrate, रिट याचिका for writs.\n"
        "  4. Cause-title — each party is a FULL descriptor (may run 2-3 lines): name, then "
        "relationship (पुत्र/पुत्री/पत्नी श्री … = S/o, D/o, W/o), age, occupation, full residence. "
        "The label ('आवेदक/आवेदिका', 'अनावेदक') sits at the LEFT, the descriptor block to its right. "
        "Put 'विरुद्ध' (EN 'Versus') CENTERED on its own line between the two parties. "
        "State respondent takes the matter's OWN State — '<राज्य> शासन द्वारा थाना ____' (blank the State as ____ if unknown; never default to M.P.).\n"
        "Add NO other header line (no court address, no coram). The frontend handles alignment "
        "(party names + विरुद्ध right, court name + case-no centered) from CSS.\n\n"
